...
chatHub.client.GetLastMessages = function (TouserID, CurrentChatMessages) {
    ...
    for (i = 0; i < CurrentChatMessages.length; i++) {
        ...
        if (i == CurrentChatMessages.length - 1) {
            if ($('#' + ctrId).children().last().html() != "<b>" + CurrentChatMessages[i].FromUserName.split('-')[0] + ": </b><span>" + CurrentChatMessages[i].Message + "</span>") {
                AllmsgHtml += "<b>" + CurrentChatMessages[i].FromUserName.split('-')[0] + ": </b><span>" + CurrentChatMessages[i].Message + "</span>";
            }
        }
        else {
            AllmsgHtml += "<b>" + CurrentChatMessages[i].FromUserName.split('-')[0] + ": </b><span>" + CurrentChatMessages[i].Message + "</span>";
...
function AddUser(chatHub, id, name, userid) {
    ...
    if (connectionid != id) {
        if ($('#' + id).length == 0) {
            code = $('<a id="' + id + '" class="col-sm-12 bg-success" > <i class=\"fa fa-user\"></i> ' + name.split('-')[0] + '<a>');
        ...
        if ($('#curruser_' + id).length == 0) {
            code = $('<div id="curruser_' + id + '" class="col-sm-12 bg-info"  ><i class=\"fa fa-user\"></i> ' + name.split('-')[0] + '<div>');
