...
function refreshOnlineUsers(onfirstTime = false) {
    //todo: Need to check and stop double time call on server
    if (onfirstTime == true) {
        $(document).find('.online-friends').load('/User/OnlineFriends', function () {
...
function createNewMessageBlockHtml(name, profilePicture, createOn, message, align, chatMessageID, status) {
    ...
    var html = '<li class="' + align + '" data-chat-message-id="' + chatMessageID + '"><span class="message"><span class="arrow"></span><span class="from">' + name + '</span>&nbsp;<span class="time">' + createOn + '</span>' + (align == 'right' ? '<span data-chat-message-id="' + chatMessageID + '" class="chat-message-status">' + status + '</span>' : '') + '<br /><span class="text">' + message + '</span></span></li>';
    return html;
}
function sendChatMessage() {
    ...
    if (chatMessage != null && chatMessage != '') {
        ...
        chat.server.sendRequest(toUserID, fromUserID, "Chat");
        /*beep(50, 520, 200);*/
